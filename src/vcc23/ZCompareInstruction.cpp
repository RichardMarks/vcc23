...
bool ZCompareInstructionSyntax::peek(const std::vector<Lexeme> &input, unsigned long start)
{
  instruction = Instruction::Unknown;
  
  if (!ParseUtils::matchInstructionToken("z", input, start))
  {
    return false;
  }
  ...
  {
    instruction = Instruction::CmpRef;
  }
...
unsigned long ZCompareInstructionSyntax::consume(
  ProgramNode &program,
